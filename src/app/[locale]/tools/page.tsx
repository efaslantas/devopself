...
import { tools, categories, getTools, getCategories } from "@/lib/data";
...
export default function ToolsPage() {
  ...
  const filtered = active === "all" ? localizedTools : localizedTools.filter((t) => t.category === active);

  return (
    <>
      <section className="border-b border-[#00f0ff]/10">
        <div className="mx-auto max-w-7xl px-4 pb-8 pt-20 sm:px-6 lg:px-8">
          <h1 className="neon-glow text-4xl font-black">{dict.tools.pageTitle}</h1>
          <p className="mt-3 max-w-2xl text-lg text-slate-400">
            {filtered.length} {dict.tools.pageDesc}
          </p>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            ...
            >
              {dict.tools.all} ({localizedTools.length})
            </button>
            {localizedCategories.map((cat) => {
              const count = localizedTools.filter((t) => t.category === cat.slug).length;
              return (
                <button
                  key={cat.slug}
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        {filtered.length === 0 && (
          <div className="py-20 text-center text-slate-500">{dict.tools.emptyState}</div>
        )}
      </section>
